...
const int W = 900;
const int H = 600;
RenderWindow app1(VideoMode(W, H), "Misha i Dima  i Asteroids!");
...
public:
	...
	{
		f.loadFromFile("images16/CyrilicOld.ttf");
		t.setFillColor(clr);
...
public:
	...
	void settings(Animation &a, int X, int Y, float Angle = 0, int radius = 1)   //верчение  корабля
	{
		anim = a;
		x = X; y = Y;
		angle = Angle;
...
void gamestart(int level)
{
	...
	{
		num_of_ast = 10;
		kol_pul = glob_ammun;  //минус потраченные
		health = glob_health;
	}
	if(level == 7)
	{
	...
	srand(time(0));
	int hits; int vrhp;
	//RenderWindow app(VideoMode(W, H), "Misha i Dima  i Asteroids!");
	//menu();//вызов меню
	app1.setFramerateLimit(60);
	Clock gameTimeClock;//переменная игрового времени, будем здесь хранить время игры 
	int gameTime = 100;//объявили игровое время, инициализировали.
	int gameTime2 = 100;//объявили игровое время, инициализировали.
	Texture t1, t2, t3, t4, t5, t6, t7, t8, t9, menucurs;
	t1.loadFromFile("images16/spaceship.png");
	menucurs.loadFromFile("images16/cursor.png");
	std::ostringstream lvl;
	lvl << level;
	String lvll;
	lvll = "images16/background" + lvl.str() + ".jpg";
	t2.loadFromFile(lvll);
	t3.loadFromFile("images16/explosions/type_C.png");
	t4.loadFromFile("images16/rock.png");
	//if (ammntn == 1)
	//{
		t5.loadFromFile("images16/fire_blue.png");
	//}
	//if (ammntn == 2)
	//{
	//	t5.loadFromFile("images16/fire_red.png");
	//}
	t6.loadFromFile("images16/rock_small.png");
	t7.loadFromFile("images16/explosions/type_B.png");
	t8.loadFromFile("images16/kot.png");
	t9.loadFromFile("images16/fire_blue.png");
	t1.setSmooth(true);
	t2.setSmooth(true);
	...
	v->settings(sVillain, 0 + 40, 0 + 70, 300, 20);
	...
	music.openFromFile("Sounds/m1.ogg");//загружаем файл
	...
	while (app1.isOpen())
	{
		...
		if (Keyboard::isKeyPressed(Keyboard::Up)) p->thrust = true;   //надо true l для полёта
		else p->thrust = false;
		...
		if (rand() % 150 == 0)   //как часто появляются новые астероиды.
		{
			asteroid *a = new asteroid();
...
void menu(int variant) 
{
	...
	{
		//RenderWindow app1(VideoMode(W, H), "Misha i Dima  i Asteroids!");
		Texture menuTexture1, menuTexture2, menuTexture3, menuTexture4, menuTextureendv, menuTextureendd, aboutTexture, menuBackground, menucurs;
		if (variant == 1 || variant == 2)
		{
			menuTexture1.loadFromFile("images16/newgame.png");
			menuTexture2.loadFromFile("images16/set.png");
			if (variant == 2)
			{
				menuTexture4.loadFromFile("images16/cont.png");
			}
			menuTexture3.loadFromFile("images16/exit.png");
			aboutTexture.loadFromFile("images16/about.png");
			menuBackground.loadFromFile("images16/backgroundmenu.jpg");
			menucurs.loadFromFile("images16/cursor.png");
		}
		if (variant == 3 || variant == 4)
		{
			
			menuTexture1.loadFromFile("images16/nextlev.png");
			menuTexture2.loadFromFile("images16/backmenu.png");
			menuTexture4.loadFromFile("images16/restart.png");
			menuTexture3.loadFromFile("images16/exit.png");
			aboutTexture.loadFromFile("images16/about.png");
			menuBackground.loadFromFile("images16/backgroundmenu.jpg");
			menucurs.loadFromFile("images16/cursor.png");
			menuTextureendv.loadFromFile("images16/vic.png");
			menuTextureendd.loadFromFile("images16/def.png");
		}
		Music music2;//создаем объект музыки
		music2.openFromFile("Sounds/m2.ogg");//загружаем файл
		Music music;//создаем объект музыки
		music.openFromFile("Sounds/m1.ogg");//загружаем файл
		//music2.stop();
		//music2.play();//воспроизводим музыку
		music2.setLoop(true);
		Sprite menu1(menuTexture1), menu2(menuTexture2), menu3(menuTexture3), menu4(menuTexture4), about(aboutTexture), menuBg(menuBackground), cursr(menucurs), menuendv(menuTextureendv), menuendd(menuTextureendd);
		bool isMenu = 1;
		int menuNum = 0;
		if (variant == 1 || variant == 2)
		{
		...
		while (isMenu)
		{
			...
			if (variant == 1 || variant == 2)
			{
				...
				if (IntRect(100, 130, 300, 100).contains(Mouse::getPosition(app1))) { menu2.setColor(Color::Blue); menuNum = 2; }
				if (IntRect(100, 230, 300, 100).contains(Mouse::getPosition(app1))) { menu3.setColor(Color::Blue); menuNum = 3; }
				if (variant == 2)
				{
			...
			{
				if (menuNum == 1) { gamestart(level); isMenu = false; isMenu1 = false; }
			...
			else if (Mouse::isButtonPressed(Mouse::Left) && variant == 3)
			{
				if (menuNum == 1 && level < 12) {
					level = level + 1;
					gamestart(level); isMenu = false; isMenu1 = false; //воспроизводим музыку
				...
				if (menuNum == 4) {
					gamestart(level); isMenu = false; isMenu1 = false;
				}
			...
			{
				if (menuNum == 1 && level < 12) {
					level = level + 1; gamestart(level); isMenu = false; isMenu1 = false; //воспроизводим музыку
				}//если нажали первую кнопку, то выходим из меню 
				if (menuNum == 2) { menu(1); }
				if (menuNum == 3) {
					app1.close(); isMenu = false; isMenu1 = false;
...
int settings()
{
	...
	Texture start, ammunitiontex, setTexture2, ammunitiontex1, ammunitiontex2, setTextureendv, menuTextureendd, aboutTexture, setBackground, menucurs;
	    start.loadFromFile("images16/start.png");
	    ammunitiontex.loadFromFile("images16/amm.png");
		ammunitiontex1.loadFromFile("images16/fire_blue.png"); ammunitiontex2.loadFromFile("images16/fire_red.png");
		setTexture2.loadFromFile("images16/backmenu.png");
		setBackground.loadFromFile("images16/backgroundmenu.jpg");
		menucurs.loadFromFile("images16/cursor.png");
		...
		while (isSet)
		{
			...
			if (Mouse::isButtonPressed(Mouse::Left) )
			{
				...
				{
					//result_ammun = 1;
					//ammun1.setColor(Color::Green);
				}
				if (setNum == 3)
				{
					//result_ammun = 2;
					//ammun2.setColor(Color::Green);
				}
			}
